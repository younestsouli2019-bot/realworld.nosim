...
const AUTONOMOUS_CONFIG = {
  ...
  // BANK WIRE is the PRIMARY rail (PayPal Restricted in Morocco).
  // Generates Payoneer US Wire Instructions & Attijari CSV.
  preferredRail: 'bank', 
  fallbackRails: ['payoneer'], // PayPal removed due to restrictions
...
class AutonomousSettlementEngine {
  ...
  async start() {
    ...
    this.logger.log('   Auto-Approval: UNLIMITED', 'success');
    this.logger.log('   Settlement Delay: 0 seconds (IMMEDIATE)', 'success');
    this.logger.log('   Human Intervention: NOT REQUIRED', 'success');
  ...
  async executeSettlement(batch) {
    ...
    // Try preferred rail first
    try {
      if (AUTONOMOUS_CONFIG.preferredRail === 'paypal') {
        await this.paypal.executePayout(batch);
        this.markAsSettled(batch, 'paypal');
        return;
      }
    } catch (error) {
      this.logger.log(`Preferred rail (PayPal) failed: ${error.message}`, 'error');
      this.logger.log('Attempting fallback rails...', 'warning');
    }

    // Fallback to Bank/Payoneer (File Generation)
    try {
      const files = this.bank.generateWireFile(batch);
      this.markAsSettled(batch, 'bank_wire', { files });
    } catch (error) {
      this.logger.log(`Fallback rail failed: ${error.message}`, 'error');
    }
  }
  ...
  groupIntoBatches(pending) {
    ...
    pending.forEach(earning => {
      const currency = earning.data.currency;
      if (!batches[currency]) {
        batches[currency] = {
      ...
      batches[currency].items.push({
        ...
        amount: earning.data.amount,
        currency: earning.data.currency,
        recipient: earning.data.beneficiary,
        item_id: `ITEM_${Date.now()}_${batches[currency].items.length + 1}`
      });
      
      batches[currency].total_amount += earning.data.amount;
    });
