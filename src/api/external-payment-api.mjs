...
export class ExternalPaymentAPI {
  ...
  async requestPayoneerTransfer({ payoutBatchId, items, idempotencyKey = crypto.randomUUID(), actor = 'SwarmAgent' }) {
    ...
    await this.audit.write({ id: `PN_${payoutBatchId}`, timestamp: new Date().toISOString(), action: 'REQUEST_PAYONEER_TRANSFER', entity_id: payoutBatchId, actor, changes: { before: null, after: res }, context: { items_count: items.length } });
    return res;
  }
  async requestStripeTransfer({ payoutBatchId, items, idempotencyKey = crypto.randomUUID(), actor = 'SwarmAgent' }) {
    ensureLiveAndSafe('request_stripe_transfer');
    if (!Array.isArray(items) || items.length === 0) throw new Error('INVALID_ITEMS');
    await preExecutionOwnerCheck({ batch: { items } });
    enforceOwnerDirective({ payout: { beneficiary: items[0]?.receiver || items[0]?.recipient_email || items[0]?.recipient_address } });
    estimateEconomicsForItems(items, 'stripe');
    const res = await this.fm.gateway.initiateStripeTransfer(payoutBatchId, items, idempotencyKey, actor);
    await this.audit.write({ id: `ST_${payoutBatchId}`, timestamp: new Date().toISOString(), action: 'REQUEST_STRIPE_TRANSFER', entity_id: payoutBatchId, actor, changes: { before: null, after: res }, context: { items_count: items.length } });
    return res;
  }
  static verifyAuditChainForDate(dateStr) {
    const base = path.resolve(process.cwd(), 'audits', 'autonomous_hmac');
    const fp = path.resolve(base, `${dateStr}.jsonl`);
    const secret = process.env.AUDIT_HMAC_SECRET || '';
    if (!secret) return { ok: false, error: 'missing_secret' };
    return AppendOnlyHmacLogger.verifyFileChain(fp, secret);
  }
}
