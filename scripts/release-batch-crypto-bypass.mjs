...
async function releaseBatchFromSource() {
  ...
  try {
    ...
    if (error.message.includes('paypal allowlist')) {
      console.log('🔍 DIAGNOSTIC: System is enforcing PayPal validation despite crypto settings.')
      console.log('💡 The external payment system appears to default to PayPal routing.')
      console.log('🛠️  SUGGESTED FIX: Modify system configuration to prioritize crypto routing.')
    }
